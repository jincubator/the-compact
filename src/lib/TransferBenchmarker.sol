    ...
    function _getNativeTokenBenchmark(bytes32 salt) internal returns (uint256 benchmark) {
        ...
        assembly ("memory-safe") {
...

            {
                ...
                transferBenchmarkCallSuccess := call(gas(), nativeTransferBenchmarker, 2, 0xa0, m, 0x20, 0)
            ...
            let warmVsColdBenchmarkCallSuccess := staticcall(gas(), warmVsColdBenchmarker, 0x60, m, 0x20, 0)
